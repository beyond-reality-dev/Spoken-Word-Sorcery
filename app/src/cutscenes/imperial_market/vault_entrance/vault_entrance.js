...
async function vaultEntrance() {
  if (getValue("imperialMarket.vaultEntrance", true).cutscenePlayed == false) {
    await printLines("app/src/cutscenes/imperial_market/vault_entrance/1.txt");
    await requireAnswer(["yes", "y"], `Aren't you the one who was sent?`);
    await printLines("app/src/cutscenes/imperial_market/vault_entrance/2.txt");
    ...
    if (response == "yes" || response == "y") {
      await printLines(
        "app/src/cutscenes/imperial_market/vault_entrance/3.txt"
      );
      changeValue(
      ...
      await printLines(
        "app/src/cutscenes/imperial_market/vault_entrance/4.txt"
      );
